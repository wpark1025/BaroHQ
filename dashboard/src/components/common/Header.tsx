...
type ModalType = 'agent' | 'team' | 'project' | 'task' | null;
...
export default function Header({ onToggleSidebar, onToggleChat }: HeaderProps) {
  ...
  const menuItems: { label: string; modal: ModalType }[] = [
    { label: 'New Agent', modal: 'agent' },
    { label: 'New Project', modal: 'project' },
    { label: 'New Task', modal: 'task' },
    { label: 'New Team', modal: 'team' },
  ];
  ...
  return (
    <>
        ...
        <AddTeamModal
          ...
          onSubmit={(teamData) => {
            const newTeam = {
              ...
              name: teamData.name,
              icon: teamData.icon,
              accent: teamData.color,
              description: teamData.description,
              floor: teams.length,
              budget: 0,
              projects: [],
              channels: [],
              agents: [teamData.leader.id],
            };
      ...
      {activeModal === 'task' && (
        <TaskCreateModal onClose={() => setActiveModal(null)} />
      )}
    </>
  );
}
